    ...
    def run(self):
        ...
        with open(self.__src, "rb") as rfs:
            with open(self.__dst, "wb") as wfs:
                ...
                _filename = _cipher.encrypt(pad(_filename.encode("utf-8"), BLOCK_SIZE))
                wfs.write(_filename)
                # 循环读取并加密、写入到新文件
...
class DecodeThread(QRunnable):
    ...
    def run(self):
        ...
        _times = (
            _filesize - KEY_LENGTH
        ) // BLOCK_SIZE - 1  # 计算读取次数，由于被填充过，所以可以整除，只有最后一次需要去除填充
        ...
        with open(self.__src, "rb") as rfs:
            ...
            with open(self.__dst, "wb") as wfs:
                ...
                try:
                    _data = unpad(_cipher.decrypt(_data), BLOCK_SIZE)  # 解密并去除填充的内容
                except ValueError:
                    # 如果解密失败，则说明文件已损坏
                    self.signals.status_sig.emit(CipherSignals.FILE_BROKEN)
                    return
